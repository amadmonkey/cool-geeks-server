...
router.get("/", isLoggedIn, async (req, res) => {
	const { accountNumber } = req.user; // get username from req.user property created by isLoggedIn middleware
	//send all todos with that user
	res.json(await Todo.find({ accountNumber }).catch((error) => res.status(400).json({ error })));
});
...
router.post("/", isLoggedIn, async (req, res) => {
	const { accountNumber } = req.user; // get username from req.user property created by isLoggedIn middleware
	req.body.accountNumber = accountNumber; // add username property to req.body
...
router.put("/:id", isLoggedIn, async (req, res) => {
	...
	res.json(
		await Todo.updateOne({ username, _id }, req.body, { new: true }).catch((error) =>
			res.status(400).json(RESPONSE.fail(403, { error }))
		)
	);
});
